...
alias ff4dinit='source ~/.local/share/ffmpeg4discord/bin/activate'
...
alias smtoff='echo off | doas tee /sys/devices/system/cpu/smt/control'
alias smton='echo on | doas tee /sys/devices/system/cpu/smt/control'
alias startx='startx "$XINITRC" -- "$XSERVERRC" vt1'
alias su='su -l'
alias vdir='vdir --color=yes'
alias vpndown='doas wg-quick down /etc/wireguard/wg-US-CO-52.conf'
alias vpnup='doas wg-quick up /etc/wireguard/wg-US-CO-52.conf'
alias webcaminitQuality='scrcpy --video-source=camera --camera-{id=0,size=4032x3024,fps=60} --no-audio --v4l2-sink=/dev/video0 --no-playback'
alias webcaminitSpeed='scrcpy --video-source=camera --camera-{id=0,high-speed,size=1920x1080,fps=240} --no-audio --v4l2-sink=/dev/video0 --no-playback'
alias webcampreview='mpv --demuxer-lavf-format=video4linux2 --demuxer-lavf-o-set=input_format=mjpeg,framerate=30 av://v4l2:/dev/pixel6a'
